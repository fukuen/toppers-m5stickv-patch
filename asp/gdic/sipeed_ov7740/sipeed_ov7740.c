...
int Sipeed_OV7740_cambus_scan_gc0328(OV7740_t *hcmr);
int Sipeed_OV7740_dvpInitIrq(void);

/*
static int cambus_readb(OV7740_t *hcmr, uint8_t reg_addr, uint8_t *reg_data)
{
...
/*
static int cambus_read_id(OV7740_t *hcmr, uint8_t addr, uint16_t *manuf_id, uint16_t *device_id)
{
...
static int cambus_scan(OV7740_t *hcmr)
{
	...
	syslog_1(LOG_NOTICE, "** device_id: %d", device_id);
		if(0xffff != device_id)
		{
			return addr ;
		}
...
{
    uint8_t reg;
    int ret = cambus_readb(hcmr->_slaveAddr, 0x0C, &reg);
    cambus_writeb(hcmr->_slaveAddr, 0x0C, OV7740_SET_MIRROR(reg, enable));

    ret = cambus_readb(hcmr->_slaveAddr, 0x16, &reg);
...
ov7740_set_special_effect(OV7740_t *hcmr, int sde)
{
    ...
    {
        case 0: // SDE_NORMAL:
            ret = cambus_readb(hcmr->_slaveAddr, 0x81, &reg);
            cambus_writeb(hcmr->_slaveAddr, 0x81, reg & 0xFE);
            ret = cambus_readb(hcmr->_slaveAddr, 0xDA, &reg);
...
ER
ov7740_sensro_gc_detect(OV7740_t *hcmr)
{
	DVP_Handle_t *hdvp = hcmr->hdvp;
	uint8_t id;
	...
	dvp_dcmi_reset(hdvp, true);
	dly_tsk(10);
	id = ov7740_cambus_scan_gc0328(hcmr);
	if(0 == id){
		return E_SYS;
	}
	else{
...
ER
ov7740_cambus_scan_gc0328(OV7740_t *hcmr)
{
	uint8_t id;
...
ov7740_set_contrast(OV7740_t *hcmr, int level)
{
    ...
    ret |= cambus_readb(hcmr->_slaveAddr, 0x81,&tmp);
    tmp |= 0x20;
    cambus_writeb(hcmr->_slaveAddr, 0x81, tmp);
    ret |= cambus_readb(hcmr->_slaveAddr, 0xDA,&tmp);
    tmp |= 0x04;
    ...
    ret |= cambus_readb(hcmr->_slaveAddr, 0xE4,&tmp);
    tmp &= 0xFB;
...
ov7740_set_auto_exposure(OV7740_t *hcmr, bool_t enable, int exposure_us)
{
    ...
    ret |= cambus_readb(hcmr->_slaveAddr, 0x13, &tmp);
    if(enable != 0)
    {
...
ov7740_get_exposure_us(OV7740_t *hcmr, int *exposure_us)
{
    ...
    ret |= cambus_readb(hcmr->_slaveAddr, 0x0F, &tmp);
    *exposure_us = tmp<<8 & 0xFF00;
    ret |= cambus_readb(hcmr->_slaveAddr, 0x10, &tmp);
...
ov7740_set_auto_whitebal(OV7740_t *hcmr, bool_t enable, float r_gain_db, float g_gain_db, float b_gain_db)
{
    ...
    ret |= cambus_readb(hcmr->_slaveAddr, 0x80, &tmp);
    if(enable != 0)
    {
...
ov7740_set_auto_gain(OV7740_t *hcmr, bool_t enable, float gain_db, float gain_db_ceiling)
{
    ...
    else
    {
        ...
        {
            ret |= cambus_readb(hcmr->_slaveAddr, 0x15, &tmp);
...
ov7740_get_gain_db(OV7740_t *hcmr, float *gain_db)
{
    ...
    ret |= cambus_readb(hcmr->_slaveAddr, 0x00, &tmp);
    gain = tmp;
    ret |= cambus_readb(hcmr->_slaveAddr, 0x15, &tmp);
